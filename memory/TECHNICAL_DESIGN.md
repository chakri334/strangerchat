...
| Date         | Change                                                                                              |
|--------------|-----------------------------------------------------------------------------------------------------|
| Feb 22 2026  | **Guest/Google identity overlap fix + cross-tab Wave-back toast.** AuthContext now exposes `loading`; `AppHeader` hides the user pill + sign-out button while `authLoading=true`, so the stale Google user cached in `localStorage` never flashes for an anonymous visitor (the exact "chakravarthi briefly visible in guest mode" bug). When `guest=true`, Home now sources the header name from `userName` only — never from `useAuth().user`. The `wave_received` socket listener moved from `PeopleTab` to `Home.js` so the toast (with an action-button **"Wave back"**) fires regardless of which tab the user is on; clicking the button posts to `/api/waves/send` and switches to People tab if it produces a mutual match. |
