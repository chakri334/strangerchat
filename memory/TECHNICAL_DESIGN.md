...
| Date         | Change                                                                                              |
|--------------|-----------------------------------------------------------------------------------------------------|
| Feb 22 2026  | **Random-chat socket-churn bug** fixed. The socket-creation `useEffect` in `Home.js` had `[guest]` as a dependency, so when a guest user clicked Connect on the landing page (which flips `guest` false→true), the existing Socket.IO connection was torn down and recreated. The server saw a real disconnect, fired `partner_disconnected` on the chat partner, and the recently-added auto-rejoin logic kicked that partner back to queue — making one user "stay connected" while the other instantly searched again. Fixed by making the socket lifecycle a one-shot (`[]` deps). Identity changes are still handled by a separate `useEffect` that re-emits `register_user` whenever `guest`/`userName`/`userGender`/`userCity`/`userInterests` change. Verified end-to-end with Playwright: two browsers click Connect → match in 1 s → stable for 5+ s → message delivery works. Incorrectly diagnosed earlier as a multi-pod state isolation issue; that infra ticket should be marked as a false alarm. |
