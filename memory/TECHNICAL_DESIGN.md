...
| Date         | Change                                                                                              |
|--------------|-----------------------------------------------------------------------------------------------------|
| Feb 22 2026  | **Telegram bot — opt-in to prevent preview/production poller fight.** Bot startup gated behind `ENABLE_TELEGRAM_BOT=true` env var. Previously, any backend with `TELEGRAM_BOT_TOKEN` set would long-poll the same bot, so preview + production were racing for every Telegram message (causing constant `409 Conflict` log spam and unpredictable matching). Now only production sets the new var and owns the bot exclusively. Preview prints `Telegram bot DISABLED` on startup. |
